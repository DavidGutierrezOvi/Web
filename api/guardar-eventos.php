<?php
/**
 * API para guardar los datos de eventos
 */
...
if (!isset($data['eventos']) || !is_array($data['eventos'])) {
    http_response_code(400);
    echo json_encode(['error' => 'Falta la lista de eventos']);
    exit;
}

$file_path = __DIR__ . '/../data/eventos.json';
...
try {
    $json_data = json_encode(['eventos' => $data['eventos']], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    if ($json_data === false) {
