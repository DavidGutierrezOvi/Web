...
/**
 * Visualiza el árbol del campeonato usando brackets-viewer.js
 * @param {string} tipo - Tipo de campeonato (mus, tute, parchis)
 */
function visualizarArbol(tipo) {
    console.log(`Visualizando árbol del campeonato de ${tipo}`);
    
    // Mostrar el contenedor de brackets-viewer y ocultar el bracket tradicional
    document.getElementById(`${tipo}-bracket`).style.display = 'none';
    document.getElementById(`${tipo}-bracket-viewer`).style.display = 'block';
    ...
    campeonatosRef.once('value')
        .then(snapshot => {
            ...
            // Preparar los datos para brackets-viewer.js
            const bracketsData = prepararDatosBrackets(campeonato, parejas, rondas, totalRondas, tipo);
            
            // Renderizar el bracket
            window.bracketsViewer.render({
                stages: bracketsData.stages,
                matches: bracketsData.matches,
                matchGames: bracketsData.matchGames,
                participants: bracketsData.participants
            }, {
                selector: `#${tipo}-bracket-viewer`,
            ...
            console.log('Árbol visualizado exitosamente');
        })
        .catch(error => {
            console.error('Error al visualizar árbol:', error);
            alert(`Error al visualizar árbol: ${error.message}`);
            // Volver a mostrar el bracket tradicional en caso de error
            document.getElementById(`${tipo}-bracket`).style.display = 'block';
            document.getElementById(`${tipo}-bracket-viewer`).style.display = 'none';
        });
}

/**
 * Prepara los datos para brackets-viewer.js
 * @param {Object} campeonato - Datos del campeonato
 * @param {Object} parejas - Datos de las parejas
 * @param {Array} rondas - Array de rondas ordenadas
 * @param {number} totalRondas - Número total de rondas
 * @param {string} tipo - Tipo de campeonato (mus, tute, parchis)
 * @returns {Object} - Datos formateados para brackets-viewer.js
 */
function prepararDatosBrackets(campeonato, parejas, rondas, totalRondas, tipo) {
    // Crear estructura de datos para brackets-viewer.js
    const stages = [
        {
            id: 1,
            name: `Campeonato de ${tipo.charAt(0).toUpperCase() + tipo.slice(1)}`,
    ...
    // Recorrer todas las parejas para crear participantes
    Object.keys(parejas).forEach(parejaId => {
        const pareja = parejas[parejaId];
        const nombre = `${pareja.nombre1 || ''} / ${pareja.nombre2 || ''}`;
        ...
        participantsMap[parejaId] = participantId;
        participantId++;
    });
    ...
    rondas.forEach((ronda, rondaIndex) => {
        const partidosRonda = campeonato[ronda];
        const roundName = getNombreRonda(ronda, totalRondas);
        
        // Para cada partido de la ronda
        partidosRonda.forEach((partido, partidoIndex) => {
            // Crear match
            const match = {
                id: matchId,
                stage_id: 1,
                round_id: parseInt(ronda),
                group_id: 1,
                round_position: partidoIndex + 1,
                status: partido.estado === 'finalizado' ? 3 : 1, // 1: pending, 3: completed
                opponent1: { id: null },
                opponent2: { id: null }
            };
            
            // Asignar participantes si existen
            if (isValidValue(partido.p1) && participantsMap[partido.p1]) {
                match.opponent1.id = participantsMap[partido.p1];
                match.opponent1.position = 1;
                
                // Si hay ganador
                if (partido.ganador === 'p1') {
                    match.opponent1.result = 'win';
                    if (match.opponent2) match.opponent2.result = 'loss';
                }
            }
            
            if (isValidValue(partido.p2) && participantsMap[partido.p2]) {
                match.opponent2.id = participantsMap[partido.p2];
                match.opponent2.position = 2;
                
                // Si hay ganador
                if (partido.ganador === 'p2') {
                    match.opponent2.result = 'win';
                    if (match.opponent1) match.opponent1.result = 'loss';
                }
            }
